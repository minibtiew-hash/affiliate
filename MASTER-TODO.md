...
8. **Next up** — run a first test Kling clip via `scripts/test_generate_video.py` to validate the animation step. Blocked on `KLING_API_KEY` (not yet set). Also needs the generated PNGs hosted at a reachable URL first (Kling doesn't accept local file paths) — no hosting/CDN solution chosen yet, needs deciding.
