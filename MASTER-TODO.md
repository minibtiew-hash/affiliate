...
13. **Demo beat realism rework 2026-07-10** — user flagged beat 3 (demo) in the first end-to-end video as looking unrealistic. Diagnosed three root causes on the real output: no visible human trigger (device acting "by itself" reads as CGI), the Kling prompt asked for a scene state change ("light state visibly changes" — the #1 morphing/artifact trigger in AI video), and nothing was anchored as still so everything drifted. Rewrote the Alpha Borong beat 3 image + Kling prompts (hand holding remote in foreground as visible cause, lamp already on in the still, one mechanical motion with camera/lighting/hand explicitly anchored static) and codified the fixes as **universal "Demo beat realism principles"** in `image-generation-animation-requirements.md` — these apply to every future product's demo beat, not just this one. Beat 3 regenerated with the new prompts 2026-07-13 ($0.26: 1 image + 1 Kling std) — the new still nails the composition (POV from bed, hand + remote foreground, device on switch mid-ground, warm lamp light) and the final video was restitched (still 8.00s, 1080x1920), sent to user for verdict on whether the realism fix worked. Running total: **$1.59**.
14. Start tagging more bad/good examples as they come in, using the scoring checklist from the requirements guide
