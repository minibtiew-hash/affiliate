...
8. **Next up** — run a first test Kling clip via `scripts/test_generate_video.py` to validate the animation step. GCS hosting confirmed working live (bucket `mike-affiliate-video-assets`). `KLING_API_KEY` set and reaching the real API (confirmed — no longer a network/auth issue, see below). **Currently blocked on: Kling account balance is $0** (`code 1102: "Account balance not enough"`, confirmed live 2026-07-09) — needs top-up before any generation can run. Test script now uses `mode="std"` instead of the pipeline's default `"pro"` for this first quality check (~40% cheaper, $0.21 vs $0.35 for 5s, same `kling-v2-6` model generation so still representative). Cost tracker fixed to be mode-aware (`KLING_COST_PER_SECOND_USD_PRO` / `_STD`) since pro and std have different confirmed rates ($0.07/s vs $0.042/s).
   - Side note on this environment: the first live attempts were blocked by this environment's network egress policy (both `kling.ai` and `api-singapore.klingai.com` got proxy-level 403s). User switched the policy to allow all domains, which required a fresh session to take effect (mid-session policy changes didn't apply to the already-running proxy) — now confirmed reaching Kling's real API.
